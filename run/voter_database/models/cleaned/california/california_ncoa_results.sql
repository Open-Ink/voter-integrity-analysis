  ...
  create or replace   view DBT_VOTER_DATA.cleaned.california_ncoa_results
  
   as (
    SELECT '2024-02-05'::date        as INPUT_FILE_DATE,
       'CA'                      AS INPUT_STATE,
       C.COUNTY_NAME             AS INPUT_COUNTY,
       NCOA.INPUT_REGISTRANT_ID  AS INPUT_VOTER_ID,
       ...
       NCOA.HOUSEHOLD_ID,
       NCOA.INDIVIDUAL_ID
FROM RAW.CALIFORNIA_NCOA_RESULTS_2024_02_05 NCOA
         INNER JOIN DBT_VOTER_DATA.cleaned.california_counties C
                    ON NCOA.INPUT_COUNTY_CODE = C.COUNTY_CODE
  );
